...
namespace misc
{
	...
	public:
		...
		int32_t draw(const app::CharacteristicCurve<int32_t, int32_t> &o_data_table, char* pc_buffer, size_t u_buffer_size);
	...
	private:
		...
		auto get_absolute_cell(int32_t x, int32_t y) -> char*;
		auto get_cell(int32_t col, int32_t row) -> char*;
