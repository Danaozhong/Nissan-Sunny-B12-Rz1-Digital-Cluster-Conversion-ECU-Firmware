...
namespace drivers
{
			...
			GPIO_TypeDef* pt_gpio_block, uint16_t u16_gpio_pin)
	{
		...
		u32_prescaler_value = static_cast<uint32_t>(SystemCoreClock / 1000000) - 1; // 171km/h  -- 721Hz
		...
		*/
		...
		o_timer_handle.Init.Period            = PERIOD_VALUE;
		...
		if (HAL_TIM_PWM_ConfigChannel(&o_timer_handle, &sConfig, u32_timer_channel) != HAL_OK)
			{
			/* Configuration Error */
			Error_Handler();
		}
		else
		{
			...
			/* Start channel 1 */
			if (HAL_TIM_PWM_Start(&o_timer_handle, u32_timer_channel) != HAL_OK)
			{
